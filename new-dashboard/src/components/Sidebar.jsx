...
export default function Sidebar({ user, selectedGuild, onSelectGuild, activePage, setActivePage, mobileMenuOpen, setMobileMenuOpen }) {
  ...
  const close = () => { if (typeof setMobileMenuOpen === 'function') setMobileMenuOpen(false); };
  ...
  return (
    <nav className={`sidebar ${mobileMenuOpen ? 'mobile-open' : ''}`}>
      <div className="sidebar-header dashboard-sidebar-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
        ...
        <button className="btn-icon mobile-only" onClick={close}
          style={{ background: 'none', border: 'none', color: '#DBDEE1', fontSize: '1.2rem', cursor: 'pointer' }}>
          <i className="fa-solid fa-xmark" />
        </button>
      </div>

      <div className="guild-selector" ref={dropdownRef} onClick={() => setDropdownOpen(!dropdownOpen)}>
        <img src={currentGuild?.icon || 'https://cdn.discordapp.com/embed/avatars/0.png'} alt="" className="guild-icon" />
        <span className="guild-name">{currentGuild?.name || 'Selecciona un servidor'}</span>
        <i className="fa-solid fa-chevron-down" />
        <div className={`guild-dropdown ${dropdownOpen ? 'active' : ''}`}>
          {(user?.allowedGuilds || []).map(g => (
            <div key={g.id} className="guild-option"
              onClick={(e) => { e.stopPropagation(); onSelectGuild(g.id); setDropdownOpen(false); close(); }}>
              {g.icon
              ...
              {g.memberCount && <span style={{ fontSize: '0.72rem', color: 'var(--muted)', marginLeft: 'auto' }}>{g.memberCount} miembros</span>}
            </div>
          ))}
        </div>
      </div>
          ...
          <ul className="nav-links">
            {section.items.map(item => (
              <li key={item.id} className={activePage === item.id ? 'active' : ''}
                onClick={() => { setActivePage(item.id); close(); }}>
      ...
      <div className="user-profile" style={{ marginTop: 'auto' }}>
        <img src={user?.avatar || 'https://cdn.discordapp.com/embed/avatars/0.png'} alt="" className="user-avatar" />
