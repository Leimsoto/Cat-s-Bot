...
export default function Tags({ selectedGuild }) {
  ...
  const createTag = async () => {
    ...
    try {
      ...
      setShowCreate(false);
      load();
    } catch (e) { showToast(e.message || 'Error al crear tag', 'error'); }
  ...
  return (
    <div className="ov-container animate-fade-in">
        ...
        <div className="glass-panel animate-fade-in" style={{padding:24,marginBottom:24}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            <div className="config-item" style={{marginBottom:0}}>
              ...
              />
              <span style={{fontSize:'0.73rem',color:'var(--muted)'}}>Se convertirá a minúsculas con guiones</span>
            </div>
            <div className="config-item" style={{marginBottom:0}}>
              ...
              <div className="lg-input-wrap">
                <div className="lg-input-inner">
                  ...
                  <div className="lg-input-actions">
                    <span style={{fontSize:'0.72rem',color:'rgba(200,200,220,0.4)'}}>
            ...
            <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
              <button className="btn-secondary" onClick={() => { setShowCreate(false); setForm(EMPTY_FORM); }}>Cancelar</button>
              <button className="btn-primary btn-save" onClick={createTag} disabled={creating} style={{padding:'10px 24px',borderRadius:12}}>
      ...
      <div style={{
        ...
        />
        {search && (
          <button onClick={() => setSearch('')} style={{background:'none',border:'none',cursor:'pointer',color:'var(--muted)'}}>✕</button>
        )}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="glass-panel" style={{padding:'48px',textAlign:'center'}}>
          ...
          <h3 style={{margin:'0 0 8px'}}>{search ? 'Sin resultados' : 'Sin tags configurados'}</h3>
          <p style={{color:'var(--muted)',margin:0,fontSize:'0.85rem'}}>
            {search ? 'Prueba con otro término.' : 'Crea tu primer tag con el botón de arriba.'}
          </p>
        </div>
      ) : (
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:14}}>
          {filtered.map(tag => (
            <div key={tag.name} className="glass-panel" style={{padding:18,position:'relative'}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <strong style={{fontSize:'0.95rem',display:'block',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                    /{tag.name}
                  </strong>
                  {tag.uses !== undefined && (
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius:8,padding:'5px 10px',color:'#f43f5e',cursor:'pointer',
                    fontSize:'0.76rem',fontWeight:700,flexShrink:0,
                  }}
                >✕</button>
              </div>
              <p style={{
                margin:0,color:'var(--muted)',fontSize:'0.82rem',lineHeight:1.5,
