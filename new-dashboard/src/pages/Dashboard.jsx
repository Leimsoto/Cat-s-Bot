...
function LoadingScreen() {
  return (
    <div className="loading-stage">
      <div className="loading-orb"><span>B</span></div>
      ...
      <div className="loading-steps">
        <span>Auth</span><span>Servidores</span><span>Módulos</span>
      </div>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const initialGuildId = cachedGuilds.some(g => g.id === savedGuildId) ? savedGuildId : null;
  ...
  const handleGuildSelect = (guildId) => {
    ...
    ['overview', 'config', 'radio', 'ia'].forEach(s =>
      apiGet(`/api/guilds/${guildId}/${s}`).catch(() => {})
    );
  };
  ...
  return (
    <div className="app-container">
      {showLanding && (
        <LandingOverlay guilds={user.allowedGuilds} onSelectGuild={handleGuildSelect} />
      )}
        ...
        <header className="topbar">
          <div className="topbar-left">
            <button className="mobile-only" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              style={{ background: 'none', border: 'none', color: '#DBDEE1', fontSize: '1.4rem', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
          ...
          <div className="topbar-actions">
            {selectedGuild && user && (
              <div className="mobile-guild-selector mobile-only"
                onClick={() => { localStorage.removeItem(GUILD_KEY); setShowLanding(true); }}>
                <img src={user.allowedGuilds.find(g => g.id === selectedGuild)?.icon || 'https://cdn.discordapp.com/embed/avatars/0.png'} alt="" />
                <span>{user.allowedGuilds.find(g => g.id === selectedGuild)?.name || 'Servidor'}</span>
              </div>
            )}
            {requiresGuild && (
              <button className="btn-icon" onClick={() => window.location.reload()} title="Recargar">
        ...
        <div className="content-area">
          {!showLanding && (!requiresGuild || selectedGuild) && (
            <>
              ...
              {activePage === 'ia'          && <IAConfig selectedGuild={selectedGuild} />}
              {activePage === 'radio'       && <Radio selectedGuild={selectedGuild} />}
              {activePage === 'tickets'     && <Tickets selectedGuild={selectedGuild} />}
              {activePage === 'moderation'  && <Moderation selectedGuild={selectedGuild} />}
              {activePage === 'embeds'      && <EmbedBuilder selectedGuild={selectedGuild} />}
              {activePage === 'levels'      && <Levels selectedGuild={selectedGuild} />}
              {activePage === 'autoroles'   && <Autoroles selectedGuild={selectedGuild} />}
              {activePage === 'giveaways'   && <Giveaways selectedGuild={selectedGuild} />}
              {activePage === 'tags'        && <Tags selectedGuild={selectedGuild} />}
              {activePage === 'reports'     && <Reports selectedGuild={selectedGuild} />}
              {activePage === 'schedules'   && <Schedules selectedGuild={selectedGuild} />}
              {activePage === 'voice-gen'   && <VoiceGen selectedGuild={selectedGuild} />}
          ...
          {!showLanding && requiresGuild && !selectedGuild && (
            <div className="dashboard-empty-state glass-panel">
              <div className="dashboard-empty-icon"><i className="fa-solid fa-building-shield" /></div>
              <h2>Selecciona un Servidor</h2>
              <p>Elige un servidor para ver sus configuraciones, estadísticas y módulos.</p>
              <button className="btn-primary" onClick={() => { localStorage.removeItem(GUILD_KEY); setShowLanding(true); }}>
                Elegir Servidor
              </button>
            </div>
          )}
